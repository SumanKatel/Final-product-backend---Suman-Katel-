...
class ForeignOrderController extends Controller
{
      ...
      public function newOrderProcess(Request $request)
    {
        ...
        $workstation = WorkstationUser::find($wsId);
            if ($workstation) {
                $array = array();
                $value = ProductOrderList::where('order_status','0')->where('is_foreign_order',1)->orderBy('order_datetime','desc')->first();
                if ($value) {
    ...
    public function priceUpdate(Request $request,$orderId)
    {
        $wsId = $request->wsId;
        $workstation = WorkstationUser::find($wsId);
            if ($workstation) {
                ...
                if ($order) {
                  ...
                  $order->courier_charge = $request->courier_charge;
