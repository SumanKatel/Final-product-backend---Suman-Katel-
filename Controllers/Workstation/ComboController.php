...
use App\Model\admin\ComboPackage;
...
class ComboController extends Controller
{
    public function comboList(Request $request)
    {
        ...
        $workstation=WorkstationUser::find($wsId);
            if ($workstation) {
                $array=array();
                $list=ComboPackage::orderBy('id','desc')->get();
                ...
                $result = array(
                                    'status'        =>true,
                                    'message'       => 'Combo Offer Data Fetched',
                                    'data' =>$array,
                                );
    ...
    public function comboAdd(Request $request)
    {
        $wsId = $request->wsId;
        $workstation=WorkstationUser::find($wsId);
        if ($workstation) {
        $validator=Validator::make($request->all(), [
            ...
            if(!empty($request->file('combo_image'))){
            $imagefile = uploadImageGcloud($request->file('combo_image'),'combo_image');
            $combo['image'] = $imagefile;
            }

            $comboResult = ComboPackage::create($combo);
                if ($comboResult) {
                foreach ($request->product_id as $key => $singleProduct) {
                    $comboProduct['product_id'] = $singleProduct;
                    $comboProduct['combo_id'] = $comboResult;
                    ComboPackageProduct::create($comboProduct);
                }
            if ($result) {
                    $data = array(
                        'status'  =>true,
